...
// Patches requestAnimationFrame to scale timestamps, enabling real-time slow-mo
...
export default defineContentScript({
  ...
  main() {
    ...
    if ((window as any).__slowmoPatched) return
    ...
    window.requestAnimationFrame = function slowmoRAF(callback: FrameRequestCallback): number {
    ...
    //   1. window.__slowmoToken  — read synchronously by content script at document_idle
    //   2. SLOWMO_STATUS_REPORT  — postMessage for SPA re-navigations where content
